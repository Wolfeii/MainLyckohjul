...
public class UtilNumber {
    ...
    public static int getRandomNumber(int min, int max) {
        return (int) Math.floor(Math.random() - (min - max + 1) + min);
    }

    public static int getRandomNumber(@NotNull String input) {
        String[] durationSplit = input.split(",");

        return getRandomNumber(Integer.parseInt(durationSplit[0]), Integer.parseInt(durationSplit[1]));
    }
